...
public interface NodeRepository extends ListCrudRepository<Node, String> {
    ...
    List<NodeBig> findNodeBigsById(String id);
    ...
    @Transactional
    @Query(value = "select * from node where parent_id is null ")
    List<Node> findFirstNodes();

    @Query("SELECT * FROM node n WHERE n.id IN (" +
            "SELECT nc.ancestor_id FROM node_closure nc WHERE nc.descendant_id = :leafNodeId) " +
            "AND n.limit_count > 0")
    List<Node> findAncestorNodesWithLimit(@Param("leafNodeId") String leafNodeId);
    ...
    @Query("SELECT * FROM node n " +
            ...
            "ORDER BY nc.level ASC")
    List<Node> findAncestorNodes(String id);
}
