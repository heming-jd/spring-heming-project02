...
public interface ApplicationRepository extends ListCrudRepository<Application, String> {
    ...
    @Query("SELECT COUNT(*) FROM application a " +
            ...
            ")")
    long countByStudentAndNodeAncestor(@Param("studentId") String studentId, @Param("ancestorId") String ancestorId);
    ...
    List<Application> findByStudentIdAndCategory(@Param("studentId") String studentId,
                                                 @Param("categoryId") String categoryId);

    @Query("SELECT a.* FROM application a WHERE a.student_id = :studentId AND a.leaf_node_id IN (:leafNodeIds)")
    List<Application> findByStudentIdAndLeafNodeIds(@Param("studentId") String studentId,
                                                    @Param("leafNodeIds") List<String> leafNodeIds);
}
